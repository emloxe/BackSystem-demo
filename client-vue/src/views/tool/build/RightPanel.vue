...
export default {
  ...
  computed: {
    documentLink() {
      return this.activeData_.document || 'https://element.eleme.cn/#/zh-CN/component/installation';
    },
    dateOptions() {
      if (this.activeData_.type !== undefined && this.activeData_.tag === 'el-date-picker') {
  ...
  methods: {
    ...
    onDefaultValueInput(str) {
      if (isArray(this.activeData_.defaultValue)) {
        ...
        // 字符串和数字
        this.$set(this.activeData_, 'defaultValue', isNumberStr(str) ? +str : str);
      }
    },
    ...
    rangeChange(val) {
      this.$set(
        this.activeData_,
        'defaultValue',
        val ? [this.activeData_.min, this.activeData_.max] : this.activeData_.min
      );
    },
    rateTextChange(val) {
